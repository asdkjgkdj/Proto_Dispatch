...
export default function DispatchHome({ onAction }) {
  const stamina = 10
    ...
    <div className="relative w-full h-full bg-yellow-50 p-4">
      ...
      <div className="grid grid-cols-3 grid-rows-3 gap-4 h-full">
        ...
        {BUILDINGS.map(building => {
          const isActive = selected === building.key
          return (
            <div
              key={building.key}
              onClick={() => setSelected(building.key)}
              className={`
                col-start-${building.col} row-start-${building.row}
